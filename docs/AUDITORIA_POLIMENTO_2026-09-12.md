...
- **Skips = dívida técnica documentada (2026-09-13).** A suíte VM ficou verde com **10 skips**, todos com comentário `DÍVIDA TÉCNICA` no arquivo: 3 em `test/unit/features/pdf_reader/pdfrx_viewer_adapter_test.dart` (pdfium nativo não carregava no VM), 3 em `test/web/chrome_smoke_test.dart` e 4 em `test/web/pdf_reader_offline_preserved_web_test.dart` (só rodam em `--platform chrome`, então apenas no CI).
- **Dívida quitada (2026-09-13, `chore/higiene-testes`).** `flutter test` com **0 skipped**; verificação local igual à do CI via `./scripts/test_all.sh` (README, seção «Testes»). Três commits:
  - **pdfium no VM** — `pdfium_dart` 0.2.5 procura a lib via `.dart_tool/native_assets.yaml` (convenção do `dart test`), mas o `flutter test` a gera em `build/native_assets/<os>/libpdfium.dylib`. `test/helpers/pdfium_test_init.dart` aponta `Pdfrx.pdfiumModulePath` para lá (falha na hora com instrução se faltar/não carregar — coberto por `pdfium_test_init_test.dart`); o group B3 chama em `setUpAll`, `skip` removido.
  - **Alvo Chrome** — os 7 testes web levam `@TestOn('browser')` (espelho do `@TestOn('vm')` de `web_index_perf_test.dart`): o VM nem os carrega, e `scripts/test_all.sh` roda `flutter test --platform chrome … test/web/` depois da suíte VM.
  - **Race do isar em worktree novo** — vários `flutter_tester` extraíam o xcframework ao mesmo tempo em `.dart_tool/isar_plus_test/` e o helper confiava na existência do diretório (314–318 loads falhando com «libisar_plus.a ausente»; o diretório vazio deixado para trás quebrava as rodadas seguintes). `test/helpers/native_test_artifact.dart`: lock exclusivo entre processos + trabalho em `.work_<pid>_<rand>/` + `rename` atômico; só o binário final conta como pronto. Reproduzido e provado por `test/helpers/isar_plus_native_library_race_test.dart` (6 processos `dart` sobre diretório vazio; vermelho antes, verde depois) e `scripts/check_isar_test_race.sh` (apaga o cache e roda `flutter test --concurrency=N`). Suíte completa verde em worktree recém-criado, concurrency padrão, sem workaround.
