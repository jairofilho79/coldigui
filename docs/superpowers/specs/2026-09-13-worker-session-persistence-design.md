...
## 12. Rollout (2026-09-14)

- **Worker**: migration `0012_create_user_sessions.sql` aplicada no D1 remoto; deploy Version `ecc7d03e-b887-4b43-b7b3-b773da8474bd` (branch `feat/worker-session`, depois mergeada). Verificado: `sess_` desconhecido → 401, `DELETE /api/auth/session` → 204, preflight com `DELETE` em `Allow-Methods`, JWT inválido → 401.
- **App (Pages)**: `web/integration` `751bbb9`, cache tag `c975b2de2f4c`, em `https://v2.plpcg.com`.
- **Checklist §8 no iPhone/PWA**: 1 ✅ (reabrir logado), 2 ✅ (favoritos após reabrir — só nos chips Coldigom, por design; acervo PLPCG não tem kind), 3 ✅ (logout), 4 ✅ (linha apagada no D1 → próxima ação levou ao botão de entrar, sem erro; login de novo funcionou). 5/6 cobertos pelo 1 (a sessão vem do `localStorage`; o `sessionStorage` morre ao fechar o PWA). 7 é aviso, não teste.
- **D12 encerrado em 2026-09-14** (mesmo dia): a migração do `sessionStorage` antigo foi removida — só ajudaria quem abrisse o app novo com um `id_token` ainda válido (≤ 1 h desde o último uso na versão antiga), janela desprezível. Quem tinha sessão antiga entra de novo uma vez.
