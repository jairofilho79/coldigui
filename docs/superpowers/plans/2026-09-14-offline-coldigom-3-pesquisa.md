...
- [ ] Simular «novo»: no DevTools apagar uma linha de `ColdigomPraiseCache` (ou pesquisar uma palavra que só a letra tem — o servidor faz FTS na letra, o índice local não): o card entra no fim com o chip «novo» e a linha diz «✓ Atualizado · 1 novo»; Network mostra `GET /api/plpcg/catalog` a seguir (sync via `syncAfterAdoption`) — como a etag guardada não mudou (a linha apagada não mexeu no servidor), a resposta mais provável é 304/`Noop`, não o dump inteiro; mesmo assim o índice re-hidrata direto do Isar (a linha adotada já está lá, `syncAfterAdoption` invalida a hidratação quando o resultado não é `Replaced`): repetir a pesquisa mostra o louvor na parte local sem chip, sem depender de um dump completo.
