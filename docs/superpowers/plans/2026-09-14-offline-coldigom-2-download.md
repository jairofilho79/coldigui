...
- [ ] **v2.plpcg.com**, fila mista rede→blob: faixa 1 não baixada (toca por URL de rede) seguida de faixa 2 baixada (blob:) — ambas tocam sem erro de CORS/crossOrigin (achado do review final: `crossOrigin` desliga `anonymous` se QUALQUER faixa da fila for blob:, não só a inicial).
- [ ] **v2.plpcg.com**, fila mista invertida blob→rede: faixa 1 baixada (blob:, é a que dispara o desbloqueio de gesto) seguida de faixa 2 não baixada (rede) — idem, ambas tocam.
