...
GestureDocument _doc(List<GestureItem> items) => GestureDocument(
  schemaMajor: 1,
  title: 't',
  dictionaryVersion: 1,
  items: items,
...
GestureCard _g(String trigger) => GestureCard(
  gestureId: 'c687580e7682',
  lyrics: [LyricLine(trigger: trigger, text: '')],
);
...
String _shapeOf(GestureItem item) => switch (item) {
  ...
  RepeatBlock(:final count, :final children) =>
    '${count}x[${_shape(children)}]',
...
void main() {
  test('cartões, texto e «instrumentos» passam inalterados; título e schema também', () {
    final doc = GestureDocument(
      ...
      items: [
        _g('a'),
        const TextLine('livre'),
        const InstructionCard(InstructionKind.instruments),
      ],
    );
  ...
  test('Nx repete os filhos N vezes com «kª vez» antes de cada passagem a partir da 2ª', () {
    final out = linearizeGestureDocument(
      _doc([
  ...
  test('«voltar ao coro» repete o último coro visto, com rótulo', () {
    ...
    expect(
      _shape(out.items),
      'coro g:c1 g:c2 g:v coro g:c1 g:c2 g:w coro g:c1 g:c2 final[g:f]',
    );
  });

  test('«voltar ao coro» sem coro mantém a instrução', () {
    final out = linearizeGestureDocument(
      _doc([_g('a'), const InstructionCard(InstructionKind.backToChorus)]),
    );
  ...
  test(
    '«repetir o louvor» duplica tudo que a raiz já emitiu, com «2ª vez»',
    () {
      final out = linearizeGestureDocument(
        _doc([
          _g('a'),
  ...
  test('«repetir o louvor» sem nada antes mantém a instrução', () {
    final out = linearizeGestureDocument(
      _doc([const InstructionCard(InstructionKind.repeatPraise), _g('a')]),
    );
  ...
  test('link e final são mantidos com os filhos linearizados', () {
    final out = linearizeGestureDocument(
      _doc([
        ...
        FinalBlock(
          children: [
            RepeatBlock(count: 2, children: [_g('c')]),
          ],
        ),
      ]),
    );
  ...
  test('181: o 2x da raiz rende 4 + 4 cartões com «2ª vez» no meio', () {
    ...
    expect(
      _shape(out.items),
      endsWith('g:Jesus 2ª g:Aleluia, g:glória g:Aleluia, g:Jesus'),
    );
  });
