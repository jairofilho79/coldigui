    ...
    ProviderScope(
      ...
      child: MaterialApp(
        ...
        home: Scaffold(
          body: Column(
            children: [
              if (parseGestureDocument(_read(fixture)).isNewerSchema)
...
void main() {
  testWidgets(
    '182: título em caixa alta, CORO tracejado sobre 5 cartões, 14 cartões, 2 instruções',
    (tester) async {
      await _pump(tester, '182_quero_viver.json');

      expect(
        find.text('182 - QUERO VIVER PRA SEMPRE COM JESUS'),
        findsOneWidget,
      );
      expect(
        find.byType(GestureCardTile, skipOffstage: false),
        findsNWidgets(14),
      );
      expect(
        find.byType(InstructionCardView, skipOffstage: false),
        findsNWidgets(2),
      );
      ...
      expect(
        (tester.widget<CustomPaint>(brace).painter as BracePainter).dashed,
        isTrue,
      );
      ...
      expect(
        braceRect.bottom,
        tester.getRect(find.byKey(gestureCardKey(4))).bottom,
      );
    },
  );
  ...
  testWidgets('181: chave 2x cobre os 4 últimos cartões', (tester) async {
    ...
    final brace = find.byKey(gestureBraceKey);
    expect(
      (tester.widget<CustomPaint>(brace).painter as BracePainter).label,
      '2x',
    );
    expect(
      tester.getRect(brace).top,
      tester.getRect(find.byKey(gestureCardKey(5))).top,
    );
    expect(
      tester.getRect(brace).bottom,
      tester.getRect(find.byKey(gestureCardKey(8))).bottom,
    );
  });

  testWidgets(
    'sintético: FINAL, conector dentro da chave, id inexistente vira placeholder, texto desconhecido não quebra',
    (tester) async {
      await _pump(tester, 'sintetico_final_link.json');
      expect(find.text('FINAL', skipOffstage: false), findsOneWidget);
      expect(
        find.byKey(gestureLinkConnectorKey, skipOffstage: false),
        findsOneWidget,
      );
      expect(
        find.byKey(gesturePlaceholderKey('000000000000'), skipOffstage: false),
        findsOneWidget,
      );
      expect(
        find.textContaining('hologram', skipOffstage: false),
        findsOneWidget,
      );
  ...
  testWidgets(
    'alias: documento com id deprecated mostra a figura da entrada ativa',
    (tester) async {
      final asked = <String>[];
      await tester.pumpWidget(
        ProviderScope(
  ...
  testWidgets(
    'dicionário vazio: todos os cartões viram placeholder, nada quebra',
    (tester) async {
      await _pump(
        tester,
        '182_quero_viver.json',
        dictionary: GestureDictionary.empty,
      );
      expect(
        find.byType(GestureCardTile, skipOffstage: false),
        findsNWidgets(14),
      ...
      expect(
        find.byKey(gesturePlaceholderKey('c687580e7682'), skipOffstage: false),
        findsWidgets,
      );
    },
  );
  ...
  testWidgets('schema v2 mostra o banner e renderiza', (tester) async {
    ...
    expect(
      find.text('Documento em formato mais novo; atualize o app.'),
      findsOneWidget,
    );
    expect(find.byType(GestureCardTile), findsOneWidget);
  });

  testWidgets('toque no cartão chama onCardTap com o índice do flatten', (
    tester,
  ) async {
    int? tapped;
  ...
  testWidgets('scrollToCard rola até o cartão', (tester) async {
    ...
    expect(
      tester.getRect(find.byKey(gestureCardKey(13))).top,
