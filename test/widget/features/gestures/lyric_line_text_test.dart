...
void main() {
  testWidgets('gatilho vermelho negrito + espaço + leitura preta', (
    tester,
  ) async {
    final rich = await _pump(
      tester,
      const LyricLine(trigger: 'Quero', text: 'viver'),
    );
  ...
  testWidgets('sem espaço quando a leitura começa com pontuação', (
    tester,
  ) async {
    for (final punct in [',', '.', ';', ':', '!', '?', ')', ']']) {
      final rich = await _pump(
        tester,
        LyricLine(trigger: 'a', text: '${punct}b'),
      );
      expect(_spans(rich)[1].text, '${punct}b', reason: punct);
    }
  });

  testWidgets('gatilho vazio → só a leitura, sem espaço na frente', (
    tester,
  ) async {
    final rich = await _pump(
      tester,
      const LyricLine(trigger: '', text: 'só leitura'),
    );
    final spans = _spans(rich);
  ...
  testWidgets('leitura vazia → só o gatilho', (tester) async {
    final rich = await _pump(
      tester,
      const LyricLine(trigger: 'Amém', text: ''),
    );
